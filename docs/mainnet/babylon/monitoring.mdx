...
<PageTitle
  ...
  title="Babylon Monitoring Setup"
  chainId="bbn-1"
  version="v2.2.0"
/>
...
This guide helps you set up comprehensive monitoring for your Babylon validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Babylon node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Babylon Node
...
```bash
mkdir -p $HOME/babylon-monitoring
cd $HOME/babylon-monitoring
```
...
services:
  prometheus:
    ...
    container_name: babylon-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: babylon-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: babylon-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: babylon-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="babylon-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Babylon
        annotations:
          summary: "Babylon validator is down"
          description: "Babylon validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Babylon
        annotations:
          summary: "Babylon node is not syncing"
          description: "Babylon node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Babylon
        annotations:
          summary: "Babylon validator missing blocks"
          description: "Babylon validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Babylon
        annotations:
          summary: "High memory usage on Babylon validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Babylon
        annotations:
          summary: "Low disk space on Babylon validator"
...
Import our Babylon validator dashboard from NODERS:
...
- **Process Exporter**: babylond process metrics
