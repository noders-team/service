...
This guide helps you set up comprehensive monitoring for your Arkeo validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Arkeo node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Arkeo Node
...
```bash
mkdir -p $HOME/arkeo-monitoring
cd $HOME/arkeo-monitoring
```
...
services:
  prometheus:
    ...
    container_name: arkeo-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: arkeo-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: arkeo-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="arkeo-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Arkeo
        annotations:
          summary: "Arkeo validator is down"
          description: "Arkeo validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Arkeo
        annotations:
          summary: "Arkeo node is not syncing"
          description: "Arkeo node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Arkeo
        annotations:
          summary: "Arkeo validator missing blocks"
          description: "Arkeo validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Arkeo
        annotations:
          summary: "High memory usage on Arkeo validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Arkeo
        annotations:
          summary: "Low disk space on Arkeo validator"
...
Import our Arkeo validator dashboard from NODERS:
...
- **Process Exporter**: arkeod process metrics
