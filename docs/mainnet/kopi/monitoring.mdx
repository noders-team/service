...
This guide helps you set up comprehensive monitoring for your Kopi validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Kopi node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Kopi Node
...
```bash
mkdir -p $HOME/kopi-monitoring
cd $HOME/kopi-monitoring
```
...
services:
  prometheus:
    ...
    container_name: kopi-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: kopi-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: kopi-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="kopi-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Kopi
        annotations:
          summary: "Kopi validator is down"
          description: "Kopi validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Kopi
        annotations:
          summary: "Kopi node is not syncing"
          description: "Kopi node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Kopi
        annotations:
          summary: "Kopi validator missing blocks"
          description: "Kopi validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Kopi
        annotations:
          summary: "High memory usage on Kopi validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Kopi
        annotations:
          summary: "Low disk space on Kopi validator"
...
Import our Kopi validator dashboard from NODERS:
...
- **Process Exporter**: uxkp process metrics
