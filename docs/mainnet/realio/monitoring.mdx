...
This guide helps you set up comprehensive monitoring for your Realio validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Realio node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Realio Node
...
```bash
mkdir -p $HOME/realio-monitoring
cd $HOME/realio-monitoring
```
...
services:
  prometheus:
    ...
    container_name: realio-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: realio-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: realio-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="realio-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Realio
        annotations:
          summary: "Realio validator is down"
          description: "Realio validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Realio
        annotations:
          summary: "Realio node is not syncing"
          description: "Realio node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Realio
        annotations:
          summary: "Realio validator missing blocks"
          description: "Realio validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Realio
        annotations:
          summary: "High memory usage on Realio validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Realio
        annotations:
          summary: "Low disk space on Realio validator"
...
Import our Realio validator dashboard from NODERS:
...
- **Process Exporter**: realiod process metrics
