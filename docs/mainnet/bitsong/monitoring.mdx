...
<PageTitle
  ...
  title="BitSong Monitoring Setup"
  chainId="auto"
  version="auto"
/>
...
This guide helps you set up comprehensive monitoring for your BitSong validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- BitSong node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in BitSong Node
...
```bash
mkdir -p $HOME/bitsong-monitoring
cd $HOME/bitsong-monitoring
```
...
services:
  prometheus:
    ...
    container_name: bitsong-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: bitsong-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: bitsong-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: bitsong-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="bitsong-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: BitSong
        annotations:
          summary: "BitSong validator is down"
          description: "BitSong validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: BitSong
        annotations:
          summary: "BitSong node is not syncing"
          description: "BitSong node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: BitSong
        annotations:
          summary: "BitSong validator missing blocks"
          description: "BitSong validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: BitSong
        annotations:
          summary: "High memory usage on BitSong validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: BitSong
        annotations:
          summary: "Low disk space on BitSong validator"
...
Import our BitSong validator dashboard from NODERS:
...
- **Process Exporter**: bitsongd process metrics  
