...
This guide helps you set up comprehensive monitoring for your UX validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- UX node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in UX Node
...
```bash
mkdir -p $HOME/umee-monitoring
cd $HOME/umee-monitoring
```
...
services:
  prometheus:
    ...
    container_name: umee-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: umee-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: umee-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="umee-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: UX
        annotations:
          summary: "UX validator is down"
          description: "UX validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: UX
        annotations:
          summary: "UX node is not syncing"
          description: "UX node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: UX
        annotations:
          summary: "UX validator missing blocks"
          description: "UX validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: UX
        annotations:
          summary: "High memory usage on UX validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: UX
        annotations:
          summary: "Low disk space on UX validator"
...
Import our UX validator dashboard from NODERS:
...
- **Process Exporter**: umeed process metrics  
