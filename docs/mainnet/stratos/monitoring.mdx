...
<PageTitle
  ...
  title="Stratos Monitoring Setup"
  chainId="stratos-1"
  version="v0.12.0"
/>
...
This guide helps you set up comprehensive monitoring for your Stratos validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Stratos node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Stratos Node
...
```bash
mkdir -p $HOME/stratos-monitoring
cd $HOME/stratos-monitoring
```
...
services:
  prometheus:
    ...
    container_name: stratos-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: stratos-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: stratos-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: stratos-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="stratos-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Stratos
        annotations:
          summary: "Stratos validator is down"
          description: "Stratos validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Stratos
        annotations:
          summary: "Stratos node is not syncing"
          description: "Stratos node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Stratos
        annotations:
          summary: "Stratos validator missing blocks"
          description: "Stratos validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Stratos
        annotations:
          summary: "High memory usage on Stratos validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Stratos
        annotations:
          summary: "Low disk space on Stratos validator"
...
Import our Stratos validator dashboard from NODERS:
...
- **Process Exporter**: stchaind process metrics
