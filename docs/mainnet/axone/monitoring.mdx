...
This guide helps you set up comprehensive monitoring for your Axone validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Axone node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Axone Node
...
```bash
mkdir -p $HOME/axone-monitoring
cd $HOME/axone-monitoring
```
...
services:
  prometheus:
    ...
    container_name: axone-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: axone-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: axone-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="axone-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Axone
        annotations:
          summary: "Axone validator is down"
          description: "Axone validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Axone
        annotations:
          summary: "Axone node is not syncing"
          description: "Axone node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Axone
        annotations:
          summary: "Axone validator missing blocks"
          description: "Axone validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Axone
        annotations:
          summary: "High memory usage on Axone validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Axone
        annotations:
          summary: "Low disk space on Axone validator"
...
Import our Axone validator dashboard from NODERS:
...
- **Process Exporter**: axoned process metrics
