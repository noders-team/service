...
<PageTitle
  ...
  title="Andromeda Monitoring Setup"
  chainId="andromeda-1"
  version="v2.0.0"
/>
...
This guide helps you set up comprehensive monitoring for your Andromeda validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Andromeda node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Andromeda Node
...
Restart your andromedad service:
...
```bash
mkdir -p $HOME/andromeda-monitoring
cd $HOME/andromeda-monitoring
```
...
services:
  prometheus:
    ...
    container_name: andromeda-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: andromeda-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: andromeda-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: andromeda-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="andromeda-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Andromeda
        annotations:
          summary: "Andromeda validator is down"
          description: "Andromeda validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Andromeda
        annotations:
          summary: "Andromeda node is not syncing"
          description: "Andromeda node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Andromeda
        annotations:
          summary: "Andromeda validator missing blocks"
          description: "Andromeda validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Andromeda
        annotations:
          summary: "High memory usage on Andromeda validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Andromeda
        annotations:
          summary: "Low disk space on Andromeda validator"
...
Import our Andromeda validator dashboard from NODERS:
...
- **Process Exporter**: andromedad process metrics
