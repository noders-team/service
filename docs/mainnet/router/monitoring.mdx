...
This guide helps you set up comprehensive monitoring for your Router validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Router node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Router Node
...
```bash
mkdir -p $HOME/router-monitoring
cd $HOME/router-monitoring
```
...
services:
  ...
  node-exporter:
    ...
    container_name: router-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: router-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="router-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Router
        annotations:
          summary: "Router validator is down"
          description: "Router validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Router
        annotations:
          summary: "Router node is not syncing"
          description: "Router node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Router
        annotations:
          summary: "Router validator missing blocks"
          description: "Router validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Router
        annotations:
          summary: "High memory usage on Router validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Router
        annotations:
          summary: "Low disk space on Router validator"
...
Import our Router validator dashboard from NODERS:
...
- **Process Exporter**: routerd process metrics
