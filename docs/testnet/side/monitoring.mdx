...
This guide helps you set up comprehensive monitoring for your Side validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Side node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Side Node
...
```bash
mkdir -p $HOME/side-monitoring
cd $HOME/side-monitoring
```
...
services:
  prometheus:
    ...
    container_name: side-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: side-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: side-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="side-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Side
        annotations:
          summary: "Side validator is down"
          description: "Side validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Side
        annotations:
          summary: "Side node is not syncing"
          description: "Side node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Side
        annotations:
          summary: "Side validator missing blocks"
          description: "Side validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Side
        annotations:
          summary: "High memory usage on Side validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Side
        annotations:
          summary: "Low disk space on Side validator"
...
Import our Side validator dashboard from NODERS:
...
- **Process Exporter**: sided process metrics
