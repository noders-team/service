...
This guide helps you set up comprehensive monitoring for your Warden validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Warden node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Warden Node
...
```bash
mkdir -p $HOME/warden-monitoring
cd $HOME/warden-monitoring
```
...
services:
  ...
  node-exporter:
    ...
    container_name: warden-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: warden-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="warden-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Warden
        annotations:
          summary: "Warden validator is down"
          description: "Warden validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Warden
        annotations:
          summary: "Warden node is not syncing"
          description: "Warden node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Warden
        annotations:
          summary: "Warden validator missing blocks"
          description: "Warden validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Warden
        annotations:
          summary: "High memory usage on Warden validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Warden
        annotations:
          summary: "Low disk space on Warden validator"
...
Import our Warden validator dashboard from NODERS:
...
- **Process Exporter**: wardend process metrics
