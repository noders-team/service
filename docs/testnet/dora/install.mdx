...
### Clone DoraVota repo and build dorad auto
...
mv $HOME/go/bin/dorad ~/.dorad/cosmovisor/upgrades/auto/bin/
```
...
```js
dorad init NAME_OF_YOUR_VALIDATOR --chain-id auto
```
...
```js
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"auto\"/" ~/.dorad/config/config.toml
```
