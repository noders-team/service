...
<PageTitle
  ...
  title="Nillion Monitoring Setup"
...
This guide helps you set up comprehensive monitoring for your Nillion validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Nillion node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Nillion Node
...
Restart your nilchaind service:
...
```bash
mkdir -p $HOME/nillion-monitoring
cd $HOME/nillion-monitoring
```
...
services:
  prometheus:
    ...
    container_name: nillion-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: nillion-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: nillion-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: nillion-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="nillion-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Nillion
        annotations:
          summary: "Nillion validator is down"
          description: "Nillion validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Nillion
        annotations:
          summary: "Nillion node is not syncing"
          description: "Nillion node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Nillion
        annotations:
          summary: "Nillion validator missing blocks"
          description: "Nillion validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Nillion
        annotations:
          summary: "High memory usage on Nillion validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Nillion
        annotations:
          summary: "Low disk space on Nillion validator"
...
Import our Nillion validator dashboard from NODERS:
...
- **Process Exporter**: nilchaind process metrics
