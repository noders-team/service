...
This guide helps you set up comprehensive monitoring for your Kyve validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Kyve node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Kyve Node
...
```bash
mkdir -p $HOME/kyve-monitoring
cd $HOME/kyve-monitoring
```
...
services:
  prometheus:
    ...
    container_name: kyve-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: kyve-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: kyve-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="kyve-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Kyve
        annotations:
          summary: "Kyve validator is down"
          description: "Kyve validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Kyve
        annotations:
          summary: "Kyve node is not syncing"
          description: "Kyve node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Kyve
        annotations:
          summary: "Kyve validator missing blocks"
          description: "Kyve validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Kyve
        annotations:
          summary: "High memory usage on Kyve validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Kyve
        annotations:
          summary: "Low disk space on Kyve validator"
...
Import our Kyve validator dashboard from NODERS:
...
- **Process Exporter**: kyved process metrics  
