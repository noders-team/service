...
### Initialize the node
```js
bandd init NAME_OF_YOUR_VALIDATOR --chain-id null
```

### Download genesis and addrbook
```js
curl -Ls https://config.noders.services/band/genesis.json > ~/.band/config/genesis.json
curl -Ls https://config.noders.services/band/addrbook.json > ~/.band/config/addrbook.json
```
### Add peers
```js
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"null@band-rpc.noders.services:null\"/" ~/.band/config/config.toml
```
