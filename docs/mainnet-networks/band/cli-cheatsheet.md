...
### Wallet balance
```js
bandd q bank balances $(bandd keys show KEY -a) --node https://band-rpc.noders.services:443
```
...
bandd tx bank send YOUR_KEY RECEIVER_ADDRESS 1000000uband \
  --chain-id  \
  --node https://band-rpc.noders.services:443 --fees 3000uband \
  --from KEY
```
...
```js
bandd query gov proposals --node https://band-rpc.noders.services:443
```
### Check vote
```js
...
### Get live peers
```js
curl -sS https://band-rpc.noders.services:443/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}'
```
