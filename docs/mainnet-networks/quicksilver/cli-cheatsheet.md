...
### Wallet balance
```js
quicksilverd q bank balances $(quicksilverd keys show KEY -a) --node https://quicksilver-rpc.noders.services:443
```
...
```js
quicksilverd query gov proposals --node https://quicksilver-rpc.noders.services:443
```
### Check vote
```js
...
### Get live peers
```js
curl -sS https://quicksilver-rpc.noders.services:443/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}'
```
