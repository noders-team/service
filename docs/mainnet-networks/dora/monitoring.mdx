...
<PageTitle
  ...
  title="DoraVota Monitoring Setup"
  chainId="vota-ash"
  version="v0.4.3"
/>
...
This guide helps you set up comprehensive monitoring for your DoraVota validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- DoraVota node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in DoraVota Node
...
```bash
mkdir -p $HOME/dora-monitoring
cd $HOME/dora-monitoring
```
...
services:
  prometheus:
    ...
    container_name: dora-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: dora-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: dora-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="dora-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: DoraVota
        annotations:
          summary: "DoraVota validator is down"
          description: "DoraVota validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: DoraVota
        annotations:
          summary: "DoraVota node is not syncing"
          description: "DoraVota node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: DoraVota
        annotations:
          summary: "DoraVota validator missing blocks"
          description: "DoraVota validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: DoraVota
        annotations:
          summary: "High memory usage on DoraVota validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: DoraVota
        annotations:
          summary: "Low disk space on DoraVota validator"
...
Import our DoraVota validator dashboard from NODERS:
...
- **Process Exporter**: dorad process metrics  
