...
This guide helps you set up comprehensive monitoring for your Jackal validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Jackal node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Jackal Node
...
```bash
mkdir -p $HOME/jackal-monitoring
cd $HOME/jackal-monitoring
```
...
services:
  prometheus:
    ...
    container_name: jackal-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: jackal-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: jackal-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="jackal-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Jackal
        annotations:
          summary: "Jackal validator is down"
          description: "Jackal validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Jackal
        annotations:
          summary: "Jackal node is not syncing"
          description: "Jackal node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Jackal
        annotations:
          summary: "Jackal validator missing blocks"
          description: "Jackal validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Jackal
        annotations:
          summary: "High memory usage on Jackal validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Jackal
        annotations:
          summary: "Low disk space on Jackal validator"
...
Import our Jackal validator dashboard from NODERS:
...
- **Process Exporter**: canined process metrics  
