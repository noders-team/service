...
binary_version: 'seq-mainnet-1.2-improved-sidecar'
...
This guide helps you set up comprehensive monitoring for your Fuel validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Fuel node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Fuel Node
...
Restart your fuelsequencerd service:
...
```bash
mkdir -p $HOME/fuel-monitoring
cd $HOME/fuel-monitoring
```
...
services:
  prometheus:
    ...
    container_name: fuel-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: fuel-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: fuel-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="fuel-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Fuel
        annotations:
          summary: "Fuel validator is down"
          description: "Fuel validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Fuel
        annotations:
          summary: "Fuel node is not syncing"
          description: "Fuel node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Fuel
        annotations:
          summary: "Fuel validator missing blocks"
          description: "Fuel validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Fuel
        annotations:
          summary: "High memory usage on Fuel validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Fuel
        annotations:
          summary: "Low disk space on Fuel validator"
...
Import our Fuel validator dashboard from NODERS:
...
- **Process Exporter**: fuelsequencerd process metrics  
