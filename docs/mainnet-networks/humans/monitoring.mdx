...
This guide helps you set up comprehensive monitoring for your Humans validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Humans node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Humans Node
...
```bash
mkdir -p $HOME/humans-monitoring
cd $HOME/humans-monitoring
```
...
services:
  prometheus:
    ...
    container_name: humans-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: humans-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: humans-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="humans-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Humans
        annotations:
          summary: "Humans validator is down"
          description: "Humans validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Humans
        annotations:
          summary: "Humans node is not syncing"
          description: "Humans node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Humans
        annotations:
          summary: "Humans validator missing blocks"
          description: "Humans validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Humans
        annotations:
          summary: "High memory usage on Humans validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Humans
        annotations:
          summary: "Low disk space on Humans validator"
...
Import our Humans validator dashboard from NODERS:
...
- **Process Exporter**: humansd process metrics
