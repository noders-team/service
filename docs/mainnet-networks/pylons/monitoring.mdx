...
This guide helps you set up comprehensive monitoring for your Pylons validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Pylons node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Pylons Node
...
```bash
mkdir -p $HOME/pylons-monitoring
cd $HOME/pylons-monitoring
```
...
services:
  prometheus:
    ...
    container_name: pylons-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: pylons-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: pylons-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="pylons-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Pylons
        annotations:
          summary: "Pylons validator is down"
          description: "Pylons validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Pylons
        annotations:
          summary: "Pylons node is not syncing"
          description: "Pylons node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Pylons
        annotations:
          summary: "Pylons validator missing blocks"
          description: "Pylons validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Pylons
        annotations:
          summary: "High memory usage on Pylons validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Pylons
        annotations:
          summary: "Low disk space on Pylons validator"
...
Import our Pylons validator dashboard from NODERS:
...
- **Process Exporter**: pylonsd process metrics  
