...
This guide helps you set up comprehensive monitoring for your Haqq validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Haqq node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Haqq Node
...
```bash
mkdir -p $HOME/haqq-monitoring
cd $HOME/haqq-monitoring
```
...
services:
  prometheus:
    ...
    container_name: haqq-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: haqq-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: haqq-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="haqq-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Haqq
        annotations:
          summary: "Haqq validator is down"
          description: "Haqq validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Haqq
        annotations:
          summary: "Haqq node is not syncing"
          description: "Haqq node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Haqq
        annotations:
          summary: "Haqq validator missing blocks"
          description: "Haqq validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Haqq
        annotations:
          summary: "High memory usage on Haqq validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Haqq
        annotations:
          summary: "Low disk space on Haqq validator"
...
Import our Haqq validator dashboard from NODERS:
...
- **Process Exporter**: haqqd process metrics  
