...
### Wallet balance
```js
osmosisd q bank balances $(osmosisd keys show KEY -a) --node https://osmosis-rpc.noders.services:443
```
...
```js
osmosisd query gov proposals --node https://osmosis-rpc.noders.services:443
```
### Check vote
```js
...
### Get live peers
```js
curl -sS https://osmosis-rpc.noders.services:443/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}'
```
