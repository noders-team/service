...
<PageTitle
  ...
  title="Dymension Monitoring Setup"
...
This guide helps you set up comprehensive monitoring for your Dymension validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Dymension node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Dymension Node
...
```bash
mkdir -p $HOME/dymension-monitoring
cd $HOME/dymension-monitoring
```
...
services:
  prometheus:
    ...
    container_name: dymension-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: dymension-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: dymension-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: dymension-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="dymension-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Dymension
        annotations:
          summary: "Dymension validator is down"
          description: "Dymension validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Dymension
        annotations:
          summary: "Dymension node is not syncing"
          description: "Dymension node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Dymension
        annotations:
          summary: "Dymension validator missing blocks"
          description: "Dymension validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Dymension
        annotations:
          summary: "High memory usage on Dymension validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Dymension
        annotations:
          summary: "Low disk space on Dymension validator"
...
Import our Dymension validator dashboard from NODERS:
...
- **Process Exporter**: dymd process metrics
