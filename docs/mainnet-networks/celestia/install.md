...
### Clone Celestia repo and build celestia-appd v1.6.0
```js
cd $HOME
...
sudo systemctl enable celestia-appd.service.service
```
...
```js
sudo systemctl start celestia-appd.service.service && sudo journalctl -u celestia-appd.service.service -f --no-hostname -o cat
```