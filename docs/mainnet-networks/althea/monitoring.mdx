...
This guide helps you set up comprehensive monitoring for your Althea validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Althea node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Althea Node
...
```bash
mkdir -p $HOME/althea-monitoring
cd $HOME/althea-monitoring
```
...
services:
  prometheus:
    ...
    container_name: althea-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: althea-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: althea-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="althea-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Althea
        annotations:
          summary: "Althea validator is down"
          description: "Althea validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Althea
        annotations:
          summary: "Althea node is not syncing"
          description: "Althea node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Althea
        annotations:
          summary: "Althea validator missing blocks"
          description: "Althea validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Althea
        annotations:
          summary: "High memory usage on Althea validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Althea
        annotations:
          summary: "Low disk space on Althea validator"
...
Import our Althea validator dashboard from NODERS:
...
- **Process Exporter**: althea process metrics  
