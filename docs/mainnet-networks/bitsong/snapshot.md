...
bitsongd tendermint unsafe-reset-all --home ~/.bitsongd --keep-addr-book
```
...
```bash
lz4 -c -d celestia_971453.tar.lz4  | tar -x -C ~/.bitsongd
```
...
```bash
curl -o - -L https://snapshots.polkachu.com/snapshots/celestia/celestia_971453.tar.lz4 | lz4 -c -d - | tar -x -C ~/.bitsongd
```
