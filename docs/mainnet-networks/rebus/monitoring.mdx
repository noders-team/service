...
This guide helps you set up comprehensive monitoring for your Rebus validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Rebus node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Rebus Node
...
```bash
mkdir -p $HOME/rebus-monitoring
cd $HOME/rebus-monitoring
```
...
services:
  prometheus:
    ...
    container_name: rebus-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: rebus-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: rebus-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="rebus-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Rebus
        annotations:
          summary: "Rebus validator is down"
          description: "Rebus validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Rebus
        annotations:
          summary: "Rebus node is not syncing"
          description: "Rebus node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Rebus
        annotations:
          summary: "Rebus validator missing blocks"
          description: "Rebus validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Rebus
        annotations:
          summary: "High memory usage on Rebus validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Rebus
        annotations:
          summary: "Low disk space on Rebus validator"
...
Import our Rebus validator dashboard from NODERS:
...
- **Process Exporter**: rebusd process metrics  
