...
<PageTitle
  ...
  title="Gitopia Monitoring Setup"
  chainId="gitopia"
  version="v5.1.0"
/>
...
This guide helps you set up comprehensive monitoring for your Gitopia validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Gitopia node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Gitopia Node
...
```bash
mkdir -p $HOME/gitopia-monitoring
cd $HOME/gitopia-monitoring
```
...
services:
  prometheus:
    ...
    container_name: gitopia-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: gitopia-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: gitopia-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: gitopia-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="gitopia-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Gitopia
        annotations:
          summary: "Gitopia validator is down"
          description: "Gitopia validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Gitopia
        annotations:
          summary: "Gitopia node is not syncing"
          description: "Gitopia node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Gitopia
        annotations:
          summary: "Gitopia validator missing blocks"
          description: "Gitopia validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Gitopia
        annotations:
          summary: "High memory usage on Gitopia validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Gitopia
        annotations:
          summary: "Low disk space on Gitopia validator"
...
Import our Gitopia validator dashboard from NODERS:
...
- **Process Exporter**: gitopiad process metrics  
