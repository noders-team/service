...
canined tendermint unsafe-reset-all --home ~/.canine --keep-addr-book
```
...
```bash
lz4 -c -d celestia_971453.tar.lz4  | tar -x -C ~/.canine
```
...
```bash
curl -o - -L https://snapshots.polkachu.com/snapshots/celestia/celestia_971453.tar.lz4 | lz4 -c -d - | tar -x -C ~/.canine
```
