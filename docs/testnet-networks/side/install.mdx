...
### Clone Side repo and build sided auto
...
mv $HOME/go/bin/sided ~/.side/cosmovisor/upgrades/auto/bin/
```
...
```js
sided init NAME_OF_YOUR_VALIDATOR --chain-id auto
```
...
```js
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"auto\"/" ~/.side/config/config.toml
```
