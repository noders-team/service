...
<PageTitle
  ...
  title="Atomone Monitoring Setup"
...
This guide helps you set up comprehensive monitoring for your Atomone validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Atomone node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Atomone Node
...
```bash
mkdir -p $HOME/atomone-monitoring
cd $HOME/atomone-monitoring
```
...
services:
  prometheus:
    ...
    container_name: atomone-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: atomone-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: atomone-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: atomone-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="atomone-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Atomone
        annotations:
          summary: "Atomone validator is down"
          description: "Atomone validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Atomone
        annotations:
          summary: "Atomone node is not syncing"
          description: "Atomone node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Atomone
        annotations:
          summary: "Atomone validator missing blocks"
          description: "Atomone validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Atomone
        annotations:
          summary: "High memory usage on Atomone validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Atomone
        annotations:
          summary: "Low disk space on Atomone validator"
...
Import our Atomone validator dashboard from NODERS:
...
- **Process Exporter**: atomoned process metrics  
