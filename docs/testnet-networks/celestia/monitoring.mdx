...
<PageTitle
  ...
  title="Celestia Monitoring Setup"
  chainId="mocha-4"
  version="v3.10.2-mocha"
/>

## Monitoring Stack

This guide helps you set up comprehensive monitoring for your Celestia validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Celestia node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Celestia Node
...
Restart your celestia-appd service:
...
```bash
mkdir -p $HOME/celestia-monitoring
cd $HOME/celestia-monitoring
```
...
services:
  prometheus:
    ...
    container_name: celestia-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: celestia-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: celestia-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: celestia-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="celestia-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Celestia
        annotations:
          summary: "Celestia validator is down"
          description: "Celestia validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Celestia
        annotations:
          summary: "Celestia node is not syncing"
          description: "Celestia node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Celestia
        annotations:
          summary: "Celestia validator missing blocks"
          description: "Celestia validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Celestia
        annotations:
          summary: "High memory usage on Celestia validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Celestia
        annotations:
          summary: "Low disk space on Celestia validator"
...
Import our Celestia validator dashboard from NODERS:
...
- **Process Exporter**: celestia-appd process metrics  
