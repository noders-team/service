...
<PageTitle
  ...
  title="Zenrock Monitoring Setup"
  chainId="gardia-9"
  version="v6.5.5"
/>
...
This guide helps you set up comprehensive monitoring for your Zenrock validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Zenrock node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Zenrock Node
...
```bash
mkdir -p $HOME/zenrock-monitoring
cd $HOME/zenrock-monitoring
```
...
services:
  prometheus:
    ...
    container_name: zenrock-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: zenrock-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: zenrock-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: zenrock-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="zenrock-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Zenrock
        annotations:
          summary: "Zenrock validator is down"
          description: "Zenrock validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Zenrock
        annotations:
          summary: "Zenrock node is not syncing"
          description: "Zenrock node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Zenrock
        annotations:
          summary: "Zenrock validator missing blocks"
          description: "Zenrock validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Zenrock
        annotations:
          summary: "High memory usage on Zenrock validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Zenrock
        annotations:
          summary: "Low disk space on Zenrock validator"
...
Import our Zenrock validator dashboard from NODERS:
...
- **Process Exporter**: zenrockd process metrics  
