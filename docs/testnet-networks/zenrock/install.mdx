...
### Clone Zenrock repo and build zenrockd v6.3.10

```js
cd $HOME
git clone https://github.com/Zenrock-Foundation/zrchain.git
cd zrchain
git checkout v6.3.10
```
...
```js
cd $HOME
mkdir -p ~/.zrchain/cosmovisor/upgrades/v6.3.10/bin
mv $HOME/go/bin/zenrockd ~/.zrchain/cosmovisor/upgrades/v6.3.10/bin/
```
...
```js
curl https://snapshots-t.noders.services/zenrock/genesis.json -o ~/.zrchain/config/genesis.json
curl https://snapshots-t.noders.services/zenrock/addrbook.json -o ~/.zrchain/config/addrbook.json
```
