...
### Clone Zenrock repo and build zenrockd v6.4.0
...
```js
cd $HOME
mkdir -p ~/.zrchain/cosmovisor/upgrades/v6.4.0/bin
mv $HOME/go/bin/zenrockd ~/.zrchain/cosmovisor/upgrades/v6.4.0/bin/
```
...
```js
zenrockd init NAME_OF_YOUR_VALIDATOR --chain-id auto
```
...
```js
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"@zenrock-t-rpc.noders.services:\"/" ~/.zrchain/config/config.toml
```
