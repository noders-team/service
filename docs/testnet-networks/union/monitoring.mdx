...
This guide helps you set up comprehensive monitoring for your Union validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Union node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Union Node
...
```bash
mkdir -p $HOME/union-monitoring
cd $HOME/union-monitoring
```
...
services:
  prometheus:
    ...
    container_name: union-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: union-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: union-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="union-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Union
        annotations:
          summary: "Union validator is down"
          description: "Union validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Union
        annotations:
          summary: "Union node is not syncing"
          description: "Union node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Union
        annotations:
          summary: "Union validator missing blocks"
          description: "Union validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Union
        annotations:
          summary: "High memory usage on Union validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Union
        annotations:
          summary: "Low disk space on Union validator"
...
Import our Union validator dashboard from NODERS:
...
- **Process Exporter**: uniond process metrics  
