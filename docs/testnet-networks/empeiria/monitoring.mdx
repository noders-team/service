...
<PageTitle
  ...
  title="Empeiria Monitoring Setup"
...
This guide helps you set up comprehensive monitoring for your Empeiria validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Empeiria node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Empeiria Node
...
```bash
mkdir -p $HOME/empeiria-monitoring
cd $HOME/empeiria-monitoring
```
...
services:
  prometheus:
    ...
    container_name: empeiria-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: empeiria-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: empeiria-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: empeiria-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="empeiria-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Empeiria
        annotations:
          summary: "Empeiria validator is down"
          description: "Empeiria validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Empeiria
        annotations:
          summary: "Empeiria node is not syncing"
          description: "Empeiria node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Empeiria
        annotations:
          summary: "Empeiria validator missing blocks"
          description: "Empeiria validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Empeiria
        annotations:
          summary: "High memory usage on Empeiria validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Empeiria
        annotations:
          summary: "Low disk space on Empeiria validator"
...
Import our Empeiria validator dashboard from NODERS:
...
- **Process Exporter**: emped process metrics  
