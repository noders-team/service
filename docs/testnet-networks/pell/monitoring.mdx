...
This guide helps you set up comprehensive monitoring for your Pell validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Pell node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Pell Node
...
Restart your pellcored service:
...
```bash
mkdir -p $HOME/pell-monitoring
cd $HOME/pell-monitoring
```
...
services:
  prometheus:
    ...
    container_name: pell-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: pell-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: pell-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="pell-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Pell
        annotations:
          summary: "Pell validator is down"
          description: "Pell validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Pell
        annotations:
          summary: "Pell node is not syncing"
          description: "Pell node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Pell
        annotations:
          summary: "Pell validator missing blocks"
          description: "Pell validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Pell
        annotations:
          summary: "High memory usage on Pell validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Pell
        annotations:
          summary: "Low disk space on Pell validator"
...
Import our Pell validator dashboard from NODERS:
...
- **Process Exporter**: pellcored process metrics  
