...
<PageTitle
  ...
  title="ZeroGravity Monitoring Setup"
...
This guide helps you set up comprehensive monitoring for your ZeroGravity validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- ZeroGravity node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in ZeroGravity Node
...
```bash
mkdir -p $HOME/og-monitoring
cd $HOME/og-monitoring
```
...
services:
  ...
  node-exporter:
    ...
    container_name: og-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: og-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="og-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: ZeroGravity
        annotations:
          summary: "ZeroGravity validator is down"
          description: "ZeroGravity validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: ZeroGravity
        annotations:
          summary: "ZeroGravity node is not syncing"
          description: "ZeroGravity node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: ZeroGravity
        annotations:
          summary: "ZeroGravity validator missing blocks"
          description: "ZeroGravity validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: ZeroGravity
        annotations:
          summary: "High memory usage on ZeroGravity validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: ZeroGravity
        annotations:
          summary: "Low disk space on ZeroGravity validator"
...
Import our ZeroGravity validator dashboard from NODERS:
...
- **Process Exporter**: 0gchaind process metrics  
