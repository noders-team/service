...
<PageTitle
  ...
  title="CrossFi Monitoring Setup"
...
This guide helps you set up comprehensive monitoring for your CrossFi validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- CrossFi node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in CrossFi Node
...
```bash
mkdir -p $HOME/crossfi-monitoring
cd $HOME/crossfi-monitoring
```
...
services:
  prometheus:
    ...
    container_name: crossfi-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: crossfi-grafana
    ports:
      - "3000:3000"
    environment:
  ...
  node-exporter:
    ...
    container_name: crossfi-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: crossfi-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="crossfi-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: CrossFi
        annotations:
          summary: "CrossFi validator is down"
          description: "CrossFi validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: CrossFi
        annotations:
          summary: "CrossFi node is not syncing"
          description: "CrossFi node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: CrossFi
        annotations:
          summary: "CrossFi validator missing blocks"
          description: "CrossFi validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: CrossFi
        annotations:
          summary: "High memory usage on CrossFi validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: CrossFi
        annotations:
          summary: "Low disk space on CrossFi validator"
...
Import our CrossFi validator dashboard from NODERS:
...
- **Process Exporter**: crossfid process metrics  
