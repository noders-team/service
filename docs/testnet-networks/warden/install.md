...
### Clone Warden repo and build wardend auto
```js
cd $HOME
...
mv $HOME/go/bin/wardend ~/.warden/cosmovisor/upgrades/auto/bin/
```
...
### Initialize the node
```js
wardend init NAME_OF_YOUR_VALIDATOR --chain-id auto
```
