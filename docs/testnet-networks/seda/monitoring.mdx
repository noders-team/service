...
This guide helps you set up comprehensive monitoring for your Seda validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Seda node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Seda Node
...
```bash
mkdir -p $HOME/seda-monitoring
cd $HOME/seda-monitoring
```
...
services:
  prometheus:
    ...
    container_name: seda-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: seda-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: seda-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="seda-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Seda
        annotations:
          summary: "Seda validator is down"
          description: "Seda validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Seda
        annotations:
          summary: "Seda node is not syncing"
          description: "Seda node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Seda
        annotations:
          summary: "Seda validator missing blocks"
          description: "Seda validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Seda
        annotations:
          summary: "High memory usage on Seda validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Seda
        annotations:
          summary: "Low disk space on Seda validator"
...
Import our Seda validator dashboard from NODERS:
...
- **Process Exporter**: sedad process metrics
