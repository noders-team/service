...
### Clone Kopi repo and build uxkp v0.6-7-g4d38756
```js
cd $HOME
...
mv $HOME/go/bin/uxkp ~/.kopid/cosmovisor/upgrades/v0.6-7-g4d38756/bin/
```
...
### Set config
```js
uxkp config chain-id kopi-test-5
...
### Initialize the node
```js
uxkp init NAME_OF_YOUR_VALIDATOR --chain-id kopi-test-5
```

### Download genesis and addrbook
```js
curl https://config-t.noders.services/kopi/genesis.json -o ~/.kopid/config/genesis.json
curl https://config-t.noders.services/kopi/addrbook.json -o ~/.kopid/config/addrbook.json
```
### Add peers
```js
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"e53411eb5216857b6625ed7970f343d003ed65a0@kopi-t-rpc.noders.services:27656\"/" ~/.kopid/config/config.toml
```
