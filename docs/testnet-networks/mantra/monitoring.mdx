...
This guide helps you set up comprehensive monitoring for your Mantra validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Mantra node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Mantra Node
...
Restart your mantrachaind service:
...
```bash
mkdir -p $HOME/mantra-monitoring
cd $HOME/mantra-monitoring
```
...
services:
  prometheus:
    ...
    container_name: mantra-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: mantra-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: mantra-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="mantra-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Mantra
        annotations:
          summary: "Mantra validator is down"
          description: "Mantra validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Mantra
        annotations:
          summary: "Mantra node is not syncing"
          description: "Mantra node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Mantra
        annotations:
          summary: "Mantra validator missing blocks"
          description: "Mantra validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Mantra
        annotations:
          summary: "High memory usage on Mantra validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Mantra
        annotations:
          summary: "Low disk space on Mantra validator"
...
Import our Mantra validator dashboard from NODERS:
...
- **Process Exporter**: mantrachaind process metrics  
