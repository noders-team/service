...
junctiond tx bank send YOUR_KEY RECEIVER_ADDRESS 1000000uamf \
  --chain-id junction \
  --node https://airchain-t-rpc.noders.services:443 --fees 3000uamf \
  --from KEY
```
...
### Get live peers
```js
curl -sS https://airchain-t-rpc.noders.services:443/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}'
```
