...
<PageTitle
  ...
  title="Sunrise Monitoring Setup"
  chainId="auto"
  version="auto"
/>
...
This guide helps you set up comprehensive monitoring for your Sunrise validator node using Prometheus and Grafana.

### Prerequisites

- Docker and Docker Compose installed
- Sunrise node running with metrics enabled
- Ports 9090 (Prometheus) and 3000 (Grafana) available

### Step 1: Enable Metrics in Sunrise Node
...
```bash
mkdir -p $HOME/sunrise-monitoring
cd $HOME/sunrise-monitoring
```
...
services:
  prometheus:
    ...
    container_name: sunrise-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  node-exporter:
    ...
    container_name: sunrise-node-exporter
    ports:
      - "9100:9100"
    command:
      - '--path.rootfs=/host'
...
groups:
  - name: sunrise-validator-alerts
    rules:
      - alert: ValidatorDown
        expr: up{job="sunrise-validator"} == 0
        for: 1m
        labels:
          severity: critical
          chain: Sunrise
        annotations:
          summary: "Sunrise validator is down"
          description: "Sunrise validator has been down for more than 1 minute"
      ...
      - alert: NodeNotSyncing
        ...
        labels:
          ...
          chain: Sunrise
        annotations:
          summary: "Sunrise node is not syncing"
          description: "Sunrise node is behind by {{ $value }} blocks"
      ...
      - alert: MissedBlocks
        ...
        labels:
          ...
          chain: Sunrise
        annotations:
          summary: "Sunrise validator missing blocks"
          description: "Sunrise validator has missed {{ $value }} blocks"
      ...
      - alert: HighMemoryUsage
        ...
        labels:
          ...
          chain: Sunrise
        annotations:
          summary: "High memory usage on Sunrise validator"
      ...
      - alert: DiskSpaceLow
        ...
        labels:
          ...
          chain: Sunrise
        annotations:
          summary: "Low disk space on Sunrise validator"
...
Import our Sunrise validator dashboard from NODERS:
...
- **Process Exporter**: sunrised process metrics  
