...
```js
nilliond q bank balances $(nilliond keys show KEY -a) --node https://nillion-t-rpc.noders.services:443
```
...
```js
nilliond query gov proposals --node https://nillion-t-rpc.noders.services:443
```
