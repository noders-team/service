...
### Clone Nillion repo and build nilchaind v0.2.1
...
```js
cd $HOME
mkdir -p ~/.nillionapp/cosmovisor/upgrades/v0.2.1/bin
mv $HOME/go/bin/nilchaind ~/.nillionapp/cosmovisor/upgrades/v0.2.1/bin/
```
...
```js
curl https://snapshots-t.noders.services/nillion/genesis.json -o ~/.nillionapp/config/genesis.json
curl https://snapshots-t.noders.services/nillion/addrbook.json -o ~/.nillionapp/config/addrbook.json
```
